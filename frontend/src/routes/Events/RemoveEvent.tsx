...
const RemoveEvent: React.FC = () => {
  ...
  const eventID = state.table.eventsTable.selected;
  const handleClickOk = () => {
    call(`/api/v1/event/${eventID}`, {method: "DELETE"});
  };

  useEffect(() => {
    if (code === 200) {
      dispatch({type: "REMOVED_EVENT"});
    ...
    <div className="p-2">
      <div className="bg-white grid gap-1 grid-cols-1 p-1 rounded-lg shadow-lg">
        ...
        <div className="grid grid-flow-col auto-cols-min gap-1 justify-center">
          <Button handleClick={() => navigate("/events/1", { replace: true })}>
            Cancel
          </Button>
